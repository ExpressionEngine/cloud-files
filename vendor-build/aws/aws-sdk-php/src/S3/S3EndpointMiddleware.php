...
use ExpressionEngine\Dependency\GuzzleHttp\Exception\InvalidArgumentException;
...
class S3EndpointMiddleware
{
    ...
    private function applyPathStyleEndpointCustomizations(CommandInterface $command, RequestInterface $request)
    {
        if ($command->getName() == 'WriteGetObjectResponse') {
            ...
            $fips = \ExpressionEngine\Dependency\Aws\is_fips_pseudo_region($this->region) ? "-fips" : "";
            $region = \ExpressionEngine\Dependency\Aws\strip_fips_pseudo_regions($this->region);
