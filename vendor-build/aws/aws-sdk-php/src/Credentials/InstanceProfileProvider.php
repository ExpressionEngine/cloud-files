...
namespace ExpressionEngine\Dependency\Aws\Credentials;

use ExpressionEngine\Dependency\Aws\Configuration\ConfigurationResolver;
use ExpressionEngine\Dependency\Aws\Exception\CredentialsException;
use ExpressionEngine\Dependency\Aws\Exception\InvalidJsonException;
use ExpressionEngine\Dependency\Aws\Sdk;
use ExpressionEngine\Dependency\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->client = $config['client'] ?? \ExpressionEngine\Dependency\Aws\default_http_handler();
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        $userAgent .= ' ' . \ExpressionEngine\Dependency\Aws\default_user_agent();
    ...
    {
        $isImdsV1Disabled = \ExpressionEngine\Dependency\Aws\boolean_value($this->ec2MetadataV1Disabled) ?? \ExpressionEngine\Dependency\Aws\boolean_value(ConfigurationResolver::resolve(self::CFG_EC2_METADATA_V1_DISABLED, self::DEFAULT_AWS_EC2_METADATA_V1_DISABLED, 'bool', $this->config)) ?? self::DEFAULT_AWS_EC2_METADATA_V1_DISABLED;
