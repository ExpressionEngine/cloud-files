...
namespace ExpressionEngine\Dependency\Aws\Endpoint\UseDualstackEndpoint;

use ExpressionEngine\Dependency\Aws\AbstractConfigurationProvider;
use ExpressionEngine\Dependency\Aws\CacheInterface;
use ExpressionEngine\Dependency\Aws\ConfigurationProviderInterface;
use ExpressionEngine\Dependency\Aws\Endpoint\UseDualstackEndpoint\Exception\ConfigurationException;
...
class ConfigurationProvider extends AbstractConfigurationProvider implements ConfigurationProviderInterface
{
    ...
    public static function ini($region, $profile = null, $filename = null)
    {
        ...
        return function () use($region, $profile, $filename) {
            ...
            $data = \ExpressionEngine\Dependency\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
            if ($data === \false) {
