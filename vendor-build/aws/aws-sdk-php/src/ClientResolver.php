...
use ExpressionEngine\Dependency\Aws\Auth\AuthResolver;
use ExpressionEngine\Dependency\Aws\Auth\AuthSchemeResolver;
use ExpressionEngine\Dependency\Aws\Auth\AuthSchemeResolverInterface;
use ExpressionEngine\Dependency\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use ExpressionEngine\Dependency\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use ExpressionEngine\Dependency\Aws\ClientSideMonitoring\Configuration;
use ExpressionEngine\Dependency\Aws\Configuration\ConfigurationResolver;
use ExpressionEngine\Dependency\Aws\Credentials\CredentialProvider;
...
use ExpressionEngine\Dependency\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use ExpressionEngine\Dependency\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use ExpressionEngine\Dependency\Aws\Endpoint\EndpointProvider;
use ExpressionEngine\Dependency\Aws\Endpoint\PartitionEndpointProvider;
use ExpressionEngine\Dependency\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use ExpressionEngine\Dependency\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use ExpressionEngine\Dependency\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use ExpressionEngine\Dependency\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use ExpressionEngine\Dependency\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use ExpressionEngine\Dependency\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use ExpressionEngine\Dependency\Aws\EndpointDiscovery\ConfigurationInterface;
use ExpressionEngine\Dependency\Aws\EndpointDiscovery\ConfigurationProvider;
use ExpressionEngine\Dependency\Aws\EndpointV2\EndpointDefinitionProvider;
use ExpressionEngine\Dependency\Aws\Exception\AwsException;
use ExpressionEngine\Dependency\Aws\Exception\InvalidRegionException;
use ExpressionEngine\Dependency\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use ExpressionEngine\Dependency\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use ExpressionEngine\Dependency\Aws\Signature\SignatureProvider;
use ExpressionEngine\Dependency\Aws\Token\Token;
use ExpressionEngine\Dependency\Aws\Token\TokenInterface;
use ExpressionEngine\Dependency\Aws\Token\TokenProvider;
...
class ClientResolver
{
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \ExpressionEngine\Dependency\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \ExpressionEngine\Dependency\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \ExpressionEngine\Dependency\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \ExpressionEngine\Dependency\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $args['region'] = \ExpressionEngine\Dependency\Aws\strip_fips_pseudo_regions($args['region']);
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            ...
            return new \ExpressionEngine\Dependency\Aws\EndpointV2\EndpointProviderV2($ruleset, EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
    ...
    public static function _apply_suppress_php_deprecation_warning($value, &$args)
    {
        if ($value) {
            ...
            $args['suppress_php_deprecation_warning'] = \ExpressionEngine\Dependency\Aws\boolean_value(getenv("AWS_SUPPRESS_PHP_DEPRECATION_WARNING"));
        } elseif (!empty($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \ExpressionEngine\Dependency\Aws\boolean_value($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        } elseif (!empty($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \ExpressionEngine\Dependency\Aws\boolean_value($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        }
    ...
    public static function _default_endpoint(array &$args)
    {
        ...
        $serviceIdentifier = \ExpressionEngine\Dependency\Aws\manifest($args['service'])['serviceIdentifier'];
...
EOT;
    ...
    private static function isValidService($service)
    {
        ...
        $services = \ExpressionEngine\Dependency\Aws\manifest();
