...
namespace ExpressionEngine\Dependency\GuzzleHttp\Handler;

use ExpressionEngine\Dependency\GuzzleHttp\Exception\ConnectException;
use ExpressionEngine\Dependency\GuzzleHttp\Exception\RequestException;
use ExpressionEngine\Dependency\GuzzleHttp\Promise as P;
use ExpressionEngine\Dependency\GuzzleHttp\Promise\FulfilledPromise;
...
use ExpressionEngine\Dependency\GuzzleHttp\TransferStats;
use ExpressionEngine\Dependency\GuzzleHttp\Utils;
use ExpressionEngine\Dependency\Psr\Http\Message\RequestInterface;
use ExpressionEngine\Dependency\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \ExpressionEngine\Dependency\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \ExpressionEngine\Dependency\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \ExpressionEngine\Dependency\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \ExpressionEngine\Dependency\GuzzleHttp\Psr7\Utils::streamFor($sink);
